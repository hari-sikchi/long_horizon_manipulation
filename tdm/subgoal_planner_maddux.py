...
class CEMoptimizer(object):
    ...
    def check_collision(self, config):
        r = Arm(self.env.links, config, '1-link', base=self.env.base_pos)
        return any([r.is_in_collision(obstacle) for obstacle in self.env.mad_env.static_objects])
        ...
        while ((t < self.max_iters)):
            ...
            for h in range(self.horizon+1):
                ...
                for i in range(self.N):
                    blocked[i] = max(self.check_collision(curr_state_t[i,:].detach().cpu().numpy()),blocked[i])
                # import ipdb;ipdb.set_trace()
                if(h==self.horizon):
                    #import ipdb; ipdb.set_trace()
...
if __name__=='__main__':
    ...
    tdm_model_path = "/Users/harshit/work/git/long_horizon_manipulation/data/maddux_new/pyt_save/model.pt"
    ...
    # Planner params
    horizon = 6
    timesteps_per_horizon = 6

    cem_planner = CEMoptimizer(env, tdm, horizon=horizon, timesteps_per_horizon=timesteps_per_horizon)

    start =   np.array([0.17,0.719,0.229,2.02,0])
    goal =  np.array([0.162,4.99,5.9,6.01,1])
