...
# A vector valued TDM Q function
class MLPtdmQFunction(nn.Module):

    def __init__(self, obs_dim, act_dim, hidden_sizes, activation, output_size=1):
        super().__init__()

        self.q = mlp([obs_dim + act_dim + obs_dim + 1] + list(hidden_sizes) + [obs_dim], activation)    
...
class MLPtdmActorCritic(nn.Module):
    ...
    def __init__(self, observation_space, action_space, hidden_sizes=(256,256),
        ...
        self.pi = SquashedGaussianMLPActor(obs_dim+obs_dim+1, act_dim, hidden_sizes, activation, act_limit).to(device)
    

        if special_policy is 'tdm':
            self.q1 = MLPtdmQFunction(obs_dim, act_dim, hidden_sizes,  activation,output_size=obs_dim).to(device)
            self.q2 = MLPtdmQFunction(obs_dim, act_dim, hidden_sizes,  activation,output_size=obs_dim).to(device)
