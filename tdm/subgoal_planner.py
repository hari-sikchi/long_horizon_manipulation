...
class CEMoptimizerPointNav(object):
    def __init__(self, env, tdm, horizon=5, timesteps_per_horizon=10):
        ...
        self.initial_var = 0.25
        # self.mean = np.zeros((self.sol_dim,))
        # self.mean = np.array([0.2,0.5,0.2,0.2,0.5,0.2,0.7,0.2,0.8,0.6,0.5,0.6])
        # self.mean = np.array([0.2,0.9,0.2,0.8,0.2,0.2,0.7,0.2])
        self.mean = np.array([0.5,0.2,0.7,0.2,0.8,0.6,0.5,0.6])
    ...
    def get_path(self, curr_state, goal_state):
        ...
        curr_state = np.array([curr_state.copy()] * self.N)
        goal_state = np.array([goal_state.copy()] * self.N)
        if(self.env.env.is_blocked(self.env.denormalize_obs(curr_state[0])) or self.env.env.is_blocked(self.env.denormalize_obs(goal_state[0]))):
        ...
        while ((t < self.max_iters)):
            ...
            subgoal_traj = np.clip(subgoal_traj, -1, 1)
            # transform subgoals to true scale
            subgoal_traj_scale = (subgoal_traj+1)/2 * 1
            # subgoal_traj_scale = subgoal_traj
            #print(np.min(subgoal_traj_scale), np.mean(subgoal_traj_scale), np.max(subgoal_traj_scale))
            costs = np.zeros((self.N,))
            blocked = np.zeros((self.N,))
            for h in range(self.horizon+1):
                ...
                for i in range(self.N):
                    blocked[i] = max(self.env.env.is_blocked(self.env.denormalize_obs(curr_state_t[i].detach().cpu().numpy())),blocked[i])
                # print(blocked)
                # import ipdb;ipdb.set_trace()
                if(h==self.horizon):
                    #import ipdb; ipdb.set_trace()
            ...
            #     import ipdb; ipdb.set_trace()
            print(blocked.sum())
            if(self.N-blocked.sum()<self.num_elites):
                
                var = 1.2*var
            else:

                costs = (blocked==1)*1e4 + (blocked==0)*costs
                indices = np.argsort(costs)
                print(costs[indices][:self.num_elites].mean())
                elites = subgoal_traj[indices][:self.num_elites]
                mean = np.mean(elites, axis=0)
...
if __name__=='__main__':
    ...
    tdm_model_path = "/Users/harshit/work/git/long_horizon_manipulation/data/SpiralNav_resize5_s0/pyt_save/model.pt"
    # tdm_model_path = "/Users/harshit/work/git/long_horizon_manipulation/data/PointNav3_resize1_s0/pyt_save/model.pt"
    ...
    # Planner params
    horizon = 4
    timesteps_per_horizon = 5

    cem_planner = CEMoptimizerPointNav(env, tdm, horizon=horizon, timesteps_per_horizon=timesteps_per_horizon)
    ...
    start_goal = np.concatenate((start.reshape(-1,2),goal.reshape(-1,2)),axis=0)
    env.plot_trajectory(start_goal,goal,'Spiral7x7')
    # path = cem_planner.get_path(start,goal)
    
    path = np.array([0.48137558,0.2190204,00.71618795,0.36848503,0.79026234,0.5559957,0.58309215,0.6270325])
    path_np = np.array(path).reshape(-1,2)
    

    full_path = np.concatenate((start.reshape(-1,2),path_np),axis=0)
    full_path = np.concatenate((full_path,goal.reshape(-1,2)),axis=0)
    # import ipdb;ipdb.set_trace()
    env.plot_trajectory(full_path,goal,'Spiral7x7')
